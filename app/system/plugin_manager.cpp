...
VoidResult PluginManager::installPlugin(const maki::Manifest& manifest)
{
  ...
  for (const auto& step : manifest.installationSteps)
  {
    ...
    if (step.shell == maki::Manifest::Shell::Unknown)
    {
    ...
    if (step.shell == maki::Manifest::Shell::Powershell)
    {
      install->setProgram("powershell.exe");
      install->setArguments({"-NoProfile",
                             "-ExecutionPolicy", "Bypass",
                             "-Command",
                             step.command,
                             step.args.join(' ')});
    }
    else if (step.shell == maki::Manifest::Shell::Bash)
    {
      install->setProgram("bash");
      install->setArguments({"-c",
                             step.command,
                             step.args.join(' ')});
    }
    else
