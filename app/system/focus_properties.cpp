...
{
  if (const auto* style = oclero::qlementine::appStyle())
    return FocusProperties{.message = message, .color = style->theme().statusColorInfo, .reason = FocusReason::MESSAGE};
  else
...
{
  if (const auto* style = oclero::qlementine::appStyle())
    return FocusProperties{.message = message, .color = style->theme().statusColorError, .reason = FocusReason::MESSAGE};
  else
    return FocusProperties{.message = message, .reason = FocusReason::MESSAGE};
}
...
{
  return FocusProperties{.widget = properties.widget, .color = properties.highlight, .reason = FocusReason::SIMULATION};
}
