...
public:
  ...
  /** True unless the user has renamed the node (`name_auto_generated` is false). */
  bool isNameAutoGenerated() const;
  /** Write NAME when still auto-generated; otherwise just repaint. */
  void syncAutoGeneratedName(const QString& title);
  ...
  /** Start in-place rename of the name label (sets name_auto_generated false on accept). */
  ...
  /** Returns the runtime flow with the given display name, or nullptr. */
  Flow* flowByName(const QString& flowName) const;
  /**
   * Ensures this task has a flow named "main" (Koda entry flow).
   * Creates one if missing. No-op for non-task nodes that never hold flows.
   */
  Flow* ensureMainFlow();
...
protected:
  ...
  QGraphicsProxyWidget* mRenameProxy = nullptr;
  QGraphicsItem::CacheMode mCacheModeBeforeRename = QGraphicsItem::DeviceCoordinateCache;

  void finishLabelRename();
