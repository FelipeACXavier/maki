...
class LabelRenameFilter : public QObject
{
public:
  explicit LabelRenameFilter(QLineEdit* edit, const QString& original)
      : QObject(edit)
      , mOriginal(original)
  {
  ...
  bool eventFilter(QObject* watched, QEvent* event) override
  {
    ...
    if (!edit || static_cast<QKeyEvent*>(event)->key() != Qt::Key_Escape)
      return QObject::eventFilter(watched, event);

    edit->setProperty("renameCancelled", true);
    edit->setText(mOriginal);
    edit->clearFocus();
    return true;
  }
...
{
  QPainterPath path = NodeBase::nodeShape(nodeRect());
...
bool NodeItem::isNameAutoGenerated() const
{
  const QVariant flag = getProperty(QStringLiteral("name_auto_generated"));
  return !flag.isValid() || flag.toBool();
}

void NodeItem::syncAutoGeneratedName(const QString& title)
{
  ...
  if (getProperty(ConfigKeys::NAME).toString() != title)
    setProperty(ConfigKeys::NAME, title);
  else
    update();
}
...
void NodeItem::updatePortPositions()
{
  ...
  else  // showError: keep the paired (right-of-center) slot so it does not jump when abort is hidden.
  {
    mErrorPort->setPos(centerX + halfSpan - errorSize * 0.5, topY);
  }
}
...
Flow* NodeItem::createFlow(const QString& flowName, std::shared_ptr<FlowSaveInfo> info)
{
  ...
  const QString resolvedName = flowName.isEmpty() && flowConfig ? flowConfig->getname() : flowName;
  if (Constants::isReservedMainFlowName(resolvedName))
  {
    const QString selfId = flowConfig ? flowConfig->getid() : QString();
    for (Flow* existing : mFlows)
    {
      if (!existing || !Constants::isReservedMainFlowName(existing->name()))
        continue;
      if (selfId.isEmpty() || existing->id() != selfId)
        return nullptr;
    }
  }
...
Flow* NodeItem::flowByName(const QString& flowName) const
{
  for (Flow* flow : mFlows)
  {
    if (flow && flow->name() == flowName)
      return flow;
  }
  return nullptr;
}

Flow* NodeItem::ensureMainFlow()
{
  if (Flow* existing = flowByName(Constants::MAIN_FLOW))
    return existing;

  auto info = std::make_shared<FlowSaveInfo>();
  info->setType(Types::CallType::USER);
  info->setModifiable(true);
  return createFlow(Constants::MAIN_FLOW, info);
}
...
{
  const QRectF local = labelHitRect();
  if (local.isEmpty())
    return false;
  return mapRectToScene(local).contains(scenePos);
}

void NodeItem::finishLabelRename()
{
  if (!mRenameProxy)
    return;

  if (auto* edit = qobject_cast<QLineEdit*>(mRenameProxy->widget()))
  ...
  setCacheMode(mCacheModeBeforeRename);
  delete proxy;
}
...
void NodeItem::beginRenameFromLabel()
{
  ...
  if (area.isEmpty())
    area = QRectF(nodeRect().left(), nodeRect().bottom() + 2.0, nodeRect().width(), labelFont().pointSizeF() * 3.0);
  area.translate(labelCenterOffsetX(), 0.0);

  const QFontMetricsF fm(labelFont());
  const qreal minWidth = qMax(72.0, fm.horizontalAdvance(QLatin1Char('M')) * 8.0);
  const qreal minHeight = fm.height() + 6.0;
  if (area.width() < minWidth)
  {
    const qreal extra = minWidth - area.width();
    area.adjust(-extra * 0.5, 0.0, extra * 0.5, 0.0);
  }
  if (area.height() < minHeight)
  {
    const qreal extra = minHeight - area.height();
    area.adjust(0.0, -extra * 0.5, 0.0, extra * 0.5);
  }

  auto* edit = new QLineEdit;
  edit->setText(nodeName());
  edit->setFont(labelFont());
  edit->setAlignment(Qt::AlignCenter);
  edit->setFrame(true);
  edit->setStyleSheet(
      QStringLiteral("QLineEdit { background: white; border: 1px solid #a0a0a0; border-radius: 2px; padding: 1px 3px; }"));
  edit->selectAll();
  new LabelRenameFilter(edit, nodeName());

  mCacheModeBeforeRename = cacheMode();
  ...
  mRenameProxy->setPos(area.topLeft());
  mRenameProxy->resize(area.size());
  mRenameProxy->setZValue(1000);
  mRenameProxy->setFlag(QGraphicsItem::ItemIsFocusable, true);

  QObject::connect(edit, &QLineEdit::editingFinished, edit, [this, edit]() {
    if (!mRenameProxy)
      return;

    const bool cancelled = edit->property("renameCancelled").toBool();
    if (!cancelled)
    {
...
void NodeItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event)
{
  ...
  const QString nodeId = id();
  QTimer::singleShot(0, graphicsScene, [graphicsScene, nodeId]() {
