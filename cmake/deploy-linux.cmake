...
foreach(line IN LISTS LDD_LINES)
  if(line MATCHES "=> (/[^ ]+)")
    ...
    if(basename MATCHES "^libQt6")
      set(candidate "${QT_RUNTIME_PREFIX}/lib/${basename}")
      if(EXISTS "${candidate}")
        list(APPEND BUNDLE_LIBS "${candidate}")
      endif()
    endif()
  endif()
endforeach()
...
if(BUNDLE_LIBS)
  ...
  foreach(lib IN LISTS BUNDLE_LIBS)
    ...
    set(dst "${CMAKE_INSTALL_PREFIX}/lib/${basename}")
    file(COPY_FILE "${real_lib}" "${dst}" RESULT copy_result ONLY_IF_DIFFERENT)

    message(STATUS "Copy result for ${basename} to ${dst}: ${copy_result}")
  endforeach()
else()
